...
1. **truncate_oversized_tool_results**：检测单条工具结果是否超过上下文 30%，超过则智能截断（检测尾部是否包含 error/result 关键词，决定用 head+tail 还是 head-only）
2. **compact_messages**：调用 LLM 对历史做结构化摘要（不是聊天摘要——明确提取工具调用、代码片段、决策、待办事项）
3. **truncate_history**：硬截断最早的消息，保留最近 N 轮

TS 版在此基础上增加了：
- **动态压缩阈值**：每个 tool result 降低 1% 触发阈值（最低 50%），避免频繁压缩
- **压缩质量评分**：压缩结果有 0-1 的质量分，低于 0.3 用 fallback，低于 0.2 触发回滚
- **Post-Compaction Loop Guard**：检测压缩后是否进入"压缩→溢出→再压缩"的死循环

...
### 设计选择五：Python 版用会话压缩代替长期记忆

Python 版没有跨会话记忆，它对"记不住"问题的全部回答是单会话内的上下文压缩：明确声明"不是聊天摘要器"，压缩时提取结构化信息（工具调用及结果、代码片段、决策记录、待办事项）而非"用户问了什么、Agent 答了什么"；摘要生成后有质量评分，不达标就换用无 LLM 的确定性兜底摘要。这与 TS 版的 dreaming 解决的是同一个问题的不同层面：Python 版解决"单次会话内上下文不溢出"（短期记忆），TS 版解决"跨会话知识不丢失"（长期记忆）。压缩机制的完整设计（三级降级、动态阈值、质量评分）见本项目的 02-agent-loop 设计选择三，与其他项目的横向对比见 comparisons/13-context-compaction。

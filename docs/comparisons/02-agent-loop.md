...
TS 版选择了单 while(true) + 约 30 个 let 状态变量。每次迭代调用 runEmbeddedAttemptWithBackend，然后根据返回结果在循环体内决定下一步。不拆分的原因是恢复路径之间有复杂交互——compaction 后需要重置 thinking level，rate-limit 后需要判断是否 escalate 到 model fallback——这些交互在分层架构中需要跨层通信，反而更难理解。
...
stateDiagram-v2
    ...
    P1 --> P2: token未超限
    P2 --> P3: API错误
    P3 --> P1: 恢复成功 turn_count-=1
    P3 --> [*]: 不可恢复
    P2 --> P4: stop_reason=tool_use
    P2 --> P1: max_tokens截断 追加续写
    P4 --> P1: 有工具结果
    P4 --> [*]: 无工具调用 end_turn
    P1 --> [*]: turn_count>=max_turns
```

**为什么这么选**：query_loop 需要在四种场景下被驱动——REPL 交互、CLI 一次性管道、子 agent 内部调用、单元测试 mock。纯函数 + 参数注入让同一个循环只需传入不同的 messages 列表和 call_model 闭包即可复用。代价是接线复杂度转移到上层：main.py 的 _build_engine() 需要处理循环依赖（engine_ref 列表闭包模式），不理解整条依赖链的人容易漏掉注册步骤。
...
**上下文管理策略**是第二个重要分歧。所有项目都面临"对话越长 token 越多"的问题，但触发压缩的时机和粒度不同：deer-flow 通过 SummarizationMiddleware 在中间件链中检测；hermes-agent 有预检和响应式双时机；openclaw-TS 有动态压缩阈值（每个 tool result 降低 1%）和压缩质量评分；claudecode 最简单——70% 阈值触发，连续失败 3 次后放弃。精细度与部署场景正相关：本地 CLI（claudecode）对话短，简单阈值够用；生产环境多轮工具调用（openclaw-TS）需要防止"压缩→溢出→再压缩"死循环。
...
**2. claudecode 的"可恢复错误不消耗轮次预算"（turn_count -= 1）设计解决了什么问题？如果去掉它，用户体验会怎么退化？**

参考答案方向：Agent 循环有 max_turns 限制（防止无限循环）。如果 429 限流重试也消耗轮次预算，一个频繁限流的 API 可能在 3 次重试后就耗尽 max_turns，导致用户看到"达到最大轮次"的错误——但实际上模型还没有开始做真正的工作。turn_count -= 1 让错误恢复"免费"，真正的轮次预算只计算有效的模型交互。去掉它的最坏情况：网络不稳定时用户频繁看到"轮次耗尽"而非真正的结果。claudecode 用独立的 retry_count（上限 5）防止无限重试，两个计数器解耦了"有效工作量"和"故障恢复成本"。

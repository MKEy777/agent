...
**失败语义**是并发执行背后更根本的约束，源自一条 API 硬规则：模型发出的每个工具调用都必须有一条对应的工具结果消息回传，一一配对、缺一不可——少一条整个请求会被 API 拒绝。这决定了"工具错误不能抛异常"的共同设计：claudecode 把工具执行包在异常捕获里，任何崩溃（包括任务被取消）都转成"带错误标记的结果"回填，模型看到错误信息自行决定重试还是换路；未注册的工具名（模型幻觉出来的）也返回错误结果而非中断循环。并行批次中一个工具失败时，其余工具照常完成，失败者的位置由错误结果占住——配对关系永不缺位。hermes 的并发工具跑在各自的线程池 worker 上，这带来一个隐蔽问题：中断信号是按线程标记的，主线程收到中断后必须显式扇出到所有 worker 线程，否则正在执行的工具感知不到中断。

**超时与取消传播**各家的成熟度差异明显。claudecode 给命令执行类工具设两分钟默认超时（可调但有十分钟硬上限），超时后两阶段终止：先发终止信号给进程留两秒清理时间，仍不退出再强杀——直接强杀可能留下未释放的锁文件。openclaw-TS 的取消走信号合并：每个工具执行时把"本次调用的取消信号"与"整个 run 的取消信号"合并监听，任一触发即中止，保证用户中断能穿透到最深层的工具调用。deer-flow 依赖 asyncio 的任务取消语义，配合"委派中的子代理收到取消后先等它到达终态再清理"的保护。判断标准是"工具会不会持有外部资源"：只操作内存的工具可以粗暴取消，持有子进程、文件锁、网络连接的工具必须有优雅终止路径。
...
**4. 为什么所有项目都选择"工具错误返回结果"而不是"抛异常中断循环"？超长工具输出截断后，模型想要完整内容怎么办？**

参考答案方向：两个原因叠加。协议层面，模型的每个工具调用必须有配对的结果消息，抛异常会导致配对缺失、下一次 API 请求直接被拒；产品层面，工具失败是 Agent 的常态而非异常（文件不存在、命令超时、参数错误），模型看到错误信息后往往能自我修正——把错误"喂回去"比中断循环的恢复能力强得多。截断找回的通用做法是把截断做成"可寻址的"：hermes 把超限结果写入磁盘、内联只留预览和文件路径，模型判断需要完整内容时用文件读取工具翻页回读——代价是额外的工具往返和磁盘 I/O；简单场景（claudecode 的 200KB 截断）则只在截断处注明"已截断"，赌被截掉的部分不重要。判断标准是"截断内容被后续引用的概率"：搜索类结果头部最相关、截尾无妨，编译日志的错误常在尾部、必须保留可回读通道。
